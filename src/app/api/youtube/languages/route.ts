...
async function processLanguageDetection(url: string) {
  ...
  const [availableLanguages, defaultLanguage] = await Promise.all([
    getAvailableLanguages(videoId),
    getVideoDefaultLanguage(videoId)
  ]);

  console.log(`Found ${availableLanguages.length} available languages for ${videoId}, default: ${defaultLanguage}`);
